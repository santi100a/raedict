...
export default async function command(socket: Socket) {
  try {
    ...
    if (!req.ok) {
      socket.write(`554 Error al contactar con la API de la RAE: ${req.status} ${req.statusText}\r\n`);
      return;
    }
    ...
    // Expected shape:
    // { "word": "xxx", "meaning": "...", "origin": "...", ... }

    const wotd = data.word || "(desconocida)";

    socket.write(`210 OK - Palabra del día: ${wotd}, tardó ${responseTime.toFixed(2)} ms en llegar\r\n`);
    return;
