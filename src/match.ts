...
export default async function command(socket: Socket, tokens: string[]) {
  ...
  try {
    ...
    if (!res.ok) {
      socket.write(`554 Error del servidor al buscar: ${res.status} ${res.statusText}\r\n`);
      return;
    }
    data = await res.json();
  } catch (err) {
  ...
  socket.write(`152 ${results.length} ${
        results.length === 1 ? "coincidencia encontrada" : "coincidencias encontradas"
      } para "${query}"\r\n`);
