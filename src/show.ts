...
export default function command(socket: Socket, tokens: string[]) {
  ...
  if (key === "INFO") {
    ...
    if (!rawDatabase) {
      socket.write('501 No has especificado el diccionario. Solamente tenemos "dle", por cierto\r\n');
      return;
    }
    ...
    socket.write("Diccionario de la Lengua Española, RAE, API no oficial <https://rae-api.com>\r\n");
  ...
  if (key === "STRAT" || key === "STRATEGIES") {
    socket.write("111 2 estrategias presentes\r\n");
