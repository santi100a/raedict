...
describe('MATCH module', () => {
	...
	describe('Parameter validation', () => {
		...
		it('accepts dle as dictionary', async () => {
			...
			await matchHandler(
				makeCommand('MATCH', ['dle', 'exact', 'test']),
				mockResponse,
			);
	...
	describe('Strategy mapping', () => {
		...
		it('uses prefix strategy with linear engine', async () => {
			...
			await matchHandler(
				makeCommand('MATCH', ['dle', 'prefix', 'test']),
				mockResponse,
			);
		...
		it('uses fuzzy strategy with hits engine for fuzzy keyword', async () => {
			...
			await matchHandler(
				makeCommand('MATCH', ['dle', 'fuzzy', 'test']),
				mockResponse,
			);
	...
	describe('Query handling', () => {
		...
		it('joins multi-word queries with spaces', async () => {
			...
			await matchHandler(
				makeCommand('MATCH', ['dle', 'exact', 'test', 'word']),
				mockResponse,
			);
	...
	describe('Successful matches', () => {
		...
		it('writes single match', async () => {
			(globalThis.fetch as jest.Mock).mockResolvedValue({
				ok: true,
				status: 200,
				json: async () => [{ doc: { id: 'palabra' }, hits: 15 }],
			});

			await matchHandler(
				makeCommand('MATCH', ['dle', 'fuzzy', 'palabra']),
				mockResponse,
			);
		...
		it('handles results with missing doc property gracefully', async () => {
			...
			await matchHandler(
				makeCommand('MATCH', ['dle', 'exact', 'nulldoctest']),
				mockResponse,
			); // Use unique word
	...
	describe('Integration scenarios', () => {
		it('handles complete exact match flow', async () => {
			(globalThis.fetch as jest.Mock).mockResolvedValue({
				ok: true,
				status: 200,
				json: async () => [{ doc: { id: 'exacto' }, hits: 20 }],
			});

			await matchHandler(
				makeCommand('MATCH', ['dle', '.', 'exacto']),
				mockResponse,
			);
		...
		it('handles prefix match with multi-word query', async () => {
			(globalThis.fetch as jest.Mock).mockResolvedValue({
				ok: true,
				status: 200,
				json: async () => [{ doc: { id: 'anti-aircraft' }, hits: 5 }],
			});

			await matchHandler(
				makeCommand('MATCH', ['dle', 'prefix', 'anti', 'air']),
				mockResponse,
			);
