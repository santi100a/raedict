...
describe('DEFINE module', () => {
  ...
  it('returns 420 error when JSON parsing fails', async () => {
    ...
    expect(mockResponse.error).toHaveBeenCalledWith(420, 'Error al interpretar la respuesta del servidor');
  });
  ...
  it('calls writeDefinitions when API returns data', async () => {
    ...
    expect(defs[0].dictionaryDescription).toBe('Diccionario de la Lengua Española');
    ...
    (globalThis.fetch as jest.Mock).mockResolvedValue({
      ...
      json: async () => ({
        data: {
          ...
          meanings: [
            {
              senses: [{ meaning_number: 1, pos: 'n', description: 'First meaning' }],
              conjugations: null,
            },
            {
              senses: [{ meaning_number: 1, pos: 'v', description: 'Second meaning' }],
              conjugations: null,
            },
          ],
        },
  ...
  it('handles errors during fetch gracefully', async () => {
    ...
    expect(mockResponse.error).toHaveBeenCalledWith(420, 'Error interno al obtener las definiciones');
  });
});